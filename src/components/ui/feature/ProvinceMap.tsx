...
const ProvinceMap: React.FC<ProvinceMapProps> = ({
  ...
  const handleProvinceClick = (provinceName: string) => {
    ...
    const isSelected = selectedProvinces.some(
      (p) => p.prefName === province.prefName,
    );
  ...
  const isProvinceSelected = (provinceName: string) => {
    return selectedProvinces.some(
      (province) => province.prefName === provinceName,
    );
  };
            ...
            {({ geographies }) =>
              geographies.map((geo) => (
                <Geography
                  ...
                  style={{
                    default: {
                      fill: isProvinceSelected(geo.properties.name)
                        ? "#FF5722"
                        : "#D6D6DA",
                      outline: "none",
